...
const SearchExercises = ({ setExercises, bodyPart, setBodyPart }) => {
  ...
  useEffect(() => {
    const fetchExercisesData = async () => {
      try {
        // Direct call to fetchData that internally uses Axios to hit the API endpoint.
        const bodyPartsData = await fetchData('https://exercisedb.p.rapidapi.com/exercises/bodyPartList', exerciseOptions);
  
        // Check if the data received is an array and then update the state
        if (Array.isArray(bodyPartsData)) {
          setBodyParts(['all', ...bodyPartsData]);
        } else {
          console.error('Expected an array of body parts, received:', bodyPartsData);
        }
      } catch (error) {
